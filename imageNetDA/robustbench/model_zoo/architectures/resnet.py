...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.relu(out)
        out3 = (out**2).sum(1)
        out3 = F.interpolate(out3[None,:], size =(14,14), mode='bilinear')
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        all_output = F.interpolate(all_output[None,:], size =(14,14), mode='bilinear')

        all_output, out = self.layer1((all_output,out))
        all_output, out = self.layer2((all_output,out))
        all_output, out = self.layer3((all_output,out))
        all_output, out = self.layer4((all_output,out))
        out = F.avg_pool2d(out, 4)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        return all_output, out
